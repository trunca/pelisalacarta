...
def mainlist(item):
    ...
    if config.get_setting("adult_mode") == "true": itemlist.append( Item(channel=__channel__, action="todaspelis", title="Eroticas +18",  url="http://www.repelis.tv/genero/eroticas/page/1" , thumbnail="http://www.topkamisetas.com/catalogo/images/TB0005.gif", fanart="http://www.topkamisetas.com/catalogo/images/TB0005.gif") )
    #Quito la busqueda por a̱o si no esta enabled el adultmode, porque no hay manera de filtrar los enlaces eroticos
    if config.get_setting("adult_mode") == "true": itemlist.append( Item(channel=__channel__, action="poranyo", title="Por A̱o", url="http://www.repelis.tv/fecha/2015/page/1", thumbnail="http://t3.gstatic.com/images?q=tbn:ANd9GcSkxiYXdBcI0cvBLsb_nNlz_dWXHRl2Q-ER9dPnP1gNUudhrqlR", fanart=mifan))
...
def porcateg(item):
    ...
    for scrapedurl,scrapedtitle in matches:      
        ...
        erotica = ""
        if config.get_setting("adult_mode") == "false":
            patron = '.*?/erotic.*?'
            try:
