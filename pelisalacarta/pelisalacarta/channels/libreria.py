...
def mainlist(item):
   ...
   for row in c:
       ...
       itemlist.append( Item(channel=__channel__, title=titulo, action="listseries", url=urlR, thumbnail=thumb, folder=True) )
...
def listseries(item):
    ...
    if tempActual == "" :
        ...
        if temps > 1 :
            ...
            for row in c:
                ...
                itemlist.append( Item(channel=__channel__, title=titulo, action="listseries", url=urlR,  folder=True) )
            return itemlist 
        else:
            tempActual = 1
    ...
    for row in c:
        ...
        itemlist.append( Item(channel=__channel__, title=contenido['title'], action=contenido['action'], url=urlR, server=contenido['server'], thumbnail=contenido['thumbnail'], folder=True) )
