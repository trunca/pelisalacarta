...
def episodios(item):
    ...
    paginas = len(re.compile('<a class="listpag" href="[^"]+">[^<]+</a>',re.DOTALL).findall(data))
    ...
    patron = '<div class="lista_title_uniq">Pelicula</div>.*?<div class="listbox" style="background:#3D3D3D;">.*?<a href="([^"]+)"'
    matches = re.compile(patron,re.DOTALL).findall(data)
    for url in matches:
      thumbnail = scrapedthumbnail
      plot = scrapedplot
      itemlist.append( Item(channel=__channel__, action="findvideos" , title=item.title , url=url, thumbnail=thumbnail, fanart=thumbnail, plot=plot))    
