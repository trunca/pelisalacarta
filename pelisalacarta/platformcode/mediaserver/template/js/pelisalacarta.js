...
function GetResponses(data) {
    ...
    switch (JsonResponse["Action"]) {
        case "AddItem":
            ...
            var str = JsonResponse["Title"];
            while ((resultado= re.exec(str)) !== null) {
                if (resultado.index === re.lastIndex) {
                    re.lastIndex++;
                }
                JsonResponse["Title"] = JsonResponse["Title"].replace(resultado[1],"<span style='color:"+resultado[2]+"'>")
                JsonResponse["Title"] = JsonResponse["Title"].replace(resultado[3],"</span>")
            }
            ...
            while ((resultado= re.exec(str)) !== null) {
                ...
                JsonResponse["Title"] = JsonResponse["Title"].replace(resultado[1],"<b>")
                JsonResponse["Title"] = JsonResponse["Title"].replace(resultado[2],"</b>")
            }
            ...
            for (x = 0; x < JsonResponse["ContextMenu"]["Count"]; x++) {
              Lista +=
              '<li class="Lista"><a href="javascript:void(0)" onclick="CerrarDialogos();DescargarContenido(\'' + JsonResponse["ContextMenu"]["Url" + x] +
              '\')" class="Lista"><h3>' + JsonResponse["ContextMenu"]["Title" + x] + '</h3></a></li>';
            }
            ...
            if (JsonResponse["ContextMenu"]["Count"] === 0) {
                BotonMenu = "";
                ClassMenu = "";
            }
        ...
        case "EndItems":
            if (Navegacion.length > 0) {
                if (Navegacion[Navegacion.length - 1].Url == UltimoRequest) {
                    ...
                    Navegacion.push({});
                    if (ItemFocus.children[0].children[0].style.display == "none") {
                      Navegacion[Navegacion.length - 1].InfoIMG     = ItemFocus.children[0].children[1].src
                    } else {
                      Navegacion[Navegacion.length - 1].InfoIMG     = ItemFocus.children[0].children[0].src
                    }
                    Navegacion[Navegacion.length - 1].InfoPLOT      = ItemFocus.children[2].innerHTML;
                    Navegacion[Navegacion.length - 1].InfoTITULO    = ItemFocus.children[1].innerHTML;
                ...
                Navegacion.push({});
                Navegacion[Navegacion.length - 1].InfoIMG         = "http://blog.tvalacarta.info/img/pelisalacarta-128x128";
                Navegacion[Navegacion.length - 1].InfoPLOT        = "Películas, series, documentales, anime, ... todo lo que puedas encontrar en Internet ahora desde tu media center. Incluye canales en Español, Italiano, Inglés y Rumano.";
                Navegacion[Navegacion.length - 1].InfoTITULO      = "pelisalacarta";
...
function DesCargarInfo(obj) {
    document.getElementById("Info-Img").src           = Navegacion[Navegacion.length - 1].InfoIMG
    document.getElementById("Info-Plot").innerHTML     = Navegacion[Navegacion.length - 1].InfoPLOT
    document.getElementById("Info-Title").innerHTML   = Navegacion[Navegacion.length - 1].InfoTITULO
}
...
function DescargarContenido(url) {
    AbrirLoading()
    if (url !== "") {
                ItemList ='<li class="ListItem"><a onfocus="DesCargarInfo(this)" onmouseover="this.focus()" class="ListItem" href="javascript:void(0)" onclick="Back()"><div class="ListItem"><span class="ListItem"></span><img class="ListItem" src="css/images/back.png"></div><h3 class="ListItem">Atrás...</h3><p class="ListItem"></p></a></li>'
    } else {
        ItemList = "";
    }
    UltimoRequest = url;
