...
class Acciones(object):
  ...
  def AddItem(self,Title="",Thumbnail="", Fanart="", Plot="",Url="",ContextMenu=[]):
    SetData(None)
    JsonData = {}
    JsonContext={}
    JsonContext["Count"]=0
  ...
  def AbrirConfig(self,Opciones):
    ...
    for Opcion in Opciones:
      try:
        Opcion[0] = config.get_localized_string(int(Opcion[0]))
      except:
        pass
      try:
        ops = Opcion[3].split("|")
        ...
        pass
      try:
        Opcion[8] = config.get_localized_string(int(Opcion[8]))
      except:
        pass
